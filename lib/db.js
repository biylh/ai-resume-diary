...
export const dbAuth = {
  getCurrentUser: async () => {
    if (isSupabaseConfigured()) {
      ...
      try {
        const profile = await localDb.getUserProfile(mockSession);
        return {
          ...profile,
          is_activated: profile?.is_activated || false
        };
  ...
  activateUser: async (userId, code) => {
    // Validate the activation code
    await localDb.validateCdkey(code);
    ...
    if (isSupabaseConfigured()) {
      ...
      return await localDb.updateUserProfile(userId, { is_activated: true });
    }
  }
};
