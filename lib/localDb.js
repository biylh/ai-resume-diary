...
export const localDb = {
  ...
  getGuestChatCount: async (guestId) => {
    const db = readDb();
    return db.users[guestId]?.chat_count || 0;
  },

  incrementGuestChatCount: async (guestId) => {
    const db = readDb();
    if (db.users[guestId]) {
      const current = db.users[guestId].chat_count || 0;
      db.users[guestId].chat_count = current + 1;
      writeDb(db);
      return db.users[guestId].chat_count;
    }
    return 0;
  },

  validateCdkey: async (code) => {
    const db = readDb();
    db.used_cdkeys = db.used_cdkeys || [];
    
    // Read valid cdkeys from lib/cdkeys.json
    let cdkeysJson;
    try {
      const cdkeysPath = path.join(process.cwd(), 'lib', 'cdkeys.json');
      cdkeysJson = JSON.parse(fs.readFileSync(cdkeysPath, 'utf-8'));
    } catch (err) {
      console.error("Error reading cdkeys.json:", err);
      throw new Error("服务器激活码配置文件读取失败");
    }
    
    const validCodes = cdkeysJson.codes || [];
    if (!validCodes.includes(code)) {
      throw new Error("无效的激活码");
    }
    
    if (db.used_cdkeys.includes(code)) {
      throw new Error("该激活码已被使用");
    }
    
    // Mark as used
    db.used_cdkeys.push(code);
    writeDb(db);
    return true;
  }
};
